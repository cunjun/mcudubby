# mcudubby probe sidecar

This Rust binary provides the experimental `probe-rs` execution backend for mcudubby.
...
Then configure mcudubby with the resulting executable:

```text
configure_probe(
    backend="probe-rs",
    target="STM32F103C8",
    probe_rs_sidecar_path="rust/probe-sidecar/target/release/mcudubby-probe-sidecar.exe",
)
```
