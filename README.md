...
```mermaid
flowchart LR
    AI["AI 客户端<br/>Codex / Claude Code"] --> MCP["mcudubby<br/>MCP Server"]
...
| `mcudubby` | 管理调试 Session、调用后端、执行安全检查并返回结构化结果 |
...
这些提示词可以直接交给已连接 `mcudubby` 的 AI 助手。
...
Keil 在本项目中承担工程构建、链接和可选的固件下载。`mcudubby` 不替代 Keil 编译器，
...
如果 `mcudubby` 对你的 MCU 调试工作有帮助，欢迎给项目一个 Star。
